...
int main(int argc, const char** argv) {
    ...
    auto camera_buffer_index =
        drawer.program.glGetProgramResourceIndex(GL_SHADER_STORAGE_BLOCK, "CameraBuffer");
    ...
    // glfwSetInputMode(conveyor.window.get(), GLFW_CURSOR,
    // GLFW_CURSOR_DISABLED); glfwSetInputMode(conveyor.window.get(),
    // GLFW_RAW_MOUSE_MOTION, GLFW_TRUE);

    // Material

    gnev::MaterialFactory material_factory(drawer.ctx, 4, 128, 128, 4, 128, 128, 4, 128, 128);
    ...
    gnev::MaterialInfo gravel_info = {.texture = {.diffuse_path = diffuse_path,
    ...
    chunk.mesh().bindAttribute(drawer.program.glGetAttribLocation("inPos"), 0);
    chunk.mesh().bindAttribute(drawer.program.glGetAttribLocation("inUV"), 1);
    chunk.mesh().bindAttribute(drawer.program.glGetAttribLocation("inMaterialId"), 2);
    ...
    //     std::cout << "\t" << i << ": " << *chunk.mesh().indices().get(i) <<
    //     std::endl;
    // }
    ...
    auto lights_buffer_index =
        drawer.program.glGetProgramResourceIndex(GL_SHADER_STORAGE_BLOCK, "PointLightBuffer");
    ...
    auto material_buffer_index =
        drawer.program.glGetProgramResourceIndex(GL_SHADER_STORAGE_BLOCK, "MaterialBuffer");
    ...
    material_factory.material_buffer().glBindBufferBase(GL_SHADER_STORAGE_BUFFER,
