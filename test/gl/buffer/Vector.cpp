...
public:
    ...
    buffer::Vector<int> initBuffer() {
        buffer::Vector<int> buffer(getCtx(), usage, init_size, init);
    ...
    void expectValue(const buffer::Vector<int>& buffer, std::size_t pos, int value) {
        int dst = -1199448855;
...
TEST_F(TestBufferVector, ctor) {
    auto usage = GL_DYNAMIC_COPY;
    auto size = 50;
    auto init = 101010;

    buffer::Vector<int> buffer(getCtx(), usage, size, init);
...
TEST_F(TestBufferVector, setElement) {
...
TEST_F(TestBufferVector, copyElement) {
    int value = 15;
...
TEST_F(TestBufferVector, insertElement) {
    int n = 3;
    auto expected = initStdVector();
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, pushElementBack) {
    int n = 3;
    int value = 15;
...
TEST_F(TestBufferVector, eraseElement) {
    auto expected = initStdVector();
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, setRange) {
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, copyRange) {
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, copyRangeOverlap) {
    auto buffer = initBuffer();
    std::vector<int> data{10, 20, 30};
...
TEST_F(TestBufferVector, getRange) {
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, insertRange) {
    int n = 3;
    auto expected = initStdVector();
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, pushRangeBack) {
    int n = 3;
    auto expected = initStdVector();
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, eraseRange) {
    auto expected = initStdVector();
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, getSize) {
    auto buffer = initBuffer();
    EXPECT_EQ(init_size, buffer.getSize());
...
TEST_F(TestBufferVector, reserve) {
    auto buffer = initBuffer();
...
TEST_F(TestBufferVector, shrinkToFit) {
    int n = 3;
    auto buffer = initBuffer();
    EXPECT_EQ(init_size, buffer.getCapacity());
    buffer.eraseRange(0, n);
