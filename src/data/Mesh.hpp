...
template <IsIndex I, IsVertex V> class Mesh {
public:
  static constexpr GLenum IndexEnum =
      std::is_same_v<I, GLubyte>    ? GL_UNSIGNED_BYTE
      : std::is_same_v<I, GLushort> ? GL_UNSIGNED_SHORT
      : std::is_same_v<I, GLuint>   ? GL_UNSIGNED_INT
                                    : 0;
...
void Mesh<I, V>::bind_attribute(GLint shader_loc, GLint attribute_loc) {
  ...
  if (shader_loc < 0 || attribute_loc < 0 ||
      attribute_loc >= vertex_info.count) {
    throw std::out_of_range("");
  }
  const auto attr_info = vertex_info.attributes[attribute_loc];
  _vao.glVertexArrayAttribBinding(shader_loc, 0);
  _vao.glVertexArrayAttribFormat(shader_loc, attr_info.elements, attr_info.type,
  ...
  std::cout << "bind_attribute: " << std::endl
            ...
            << "\tvertex_info.offsets[attribute_loc]"
            << vertex_info.offsets[attribute_loc] << std::endl;
}
