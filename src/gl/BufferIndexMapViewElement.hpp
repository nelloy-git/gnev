...
public:
    ...
    BufferIndexMapViewElement(WeakRef<BufferIndexMapView<T>> weak_view,
                              const T& initial = T{});
...
private:
    ...
    static Ref<GLuint>
    initIndexKeeper(const WeakRef<BufferIndexMapView<T>>& weak_view, const T& initial);
};

template <typename T>
BufferIndexMapViewElement<T>::BufferIndexMapViewElement(WeakRef<BufferIndexMapView<T>>
                                                            weak_view,
                                                        const T& initial)
    : weak_view(weak_view)
...
template <typename T>
void BufferIndexMapViewElement<T>::set(const T& src) {
    auto view_opt = weak_view.lock();
    if (not view_opt.has_value()) {
        throw std::runtime_error("");
    }
    view_opt.value()->set(index_guard, src);
}

template <typename T>
template <typename V>
void BufferIndexMapViewElement<T>::set(const V& src, GLintptr ptr_offset) {
    auto view_opt = weak_view.lock();
    if (not view_opt.has_value()) {
        throw std::runtime_error("");
    }
    view_opt.value()->set(index_guard, src, ptr_offset);
}

template <typename T>
void BufferIndexMapViewElement<T>::get(T& dst) const {
    auto view_opt = weak_view.lock();
    if (not view_opt.has_value()) {
        throw std::runtime_error("");
    }
    view_opt.value()->get(index_guard, dst);
}

template <typename T>
template <typename V>
void BufferIndexMapViewElement<T>::get(V& dst, GLintptr ptr_offset) const {
    auto view_opt = weak_view.lock();
    if (not view_opt.has_value()) {
        throw std::runtime_error("");
    }
    view_opt.value()->get(index_guard, dst, ptr_offset);
}

template <typename T>
void BufferIndexMapViewElement<T>::change(const Changer<T>& changer) {
    auto view_opt = weak_view.lock();
    if (not view_opt.has_value()) {
        throw std::runtime_error("");
    }
    view_opt.value()->change(index_guard, changer);
}

template <typename T>
template <typename V>
void BufferIndexMapViewElement<T>::change(const Changer<V>& changer,
                                          GLintptr ptr_offset) {
    auto view_opt = weak_view.lock();
    if (not view_opt.has_value()) {
        throw std::runtime_error("");
    }
    view_opt.value()->change(index_guard, changer, ptr_offset);
}

template <typename T>
Ref<GLuint>
BufferIndexMapViewElement<T>::initIndexKeeper(const WeakRef<BufferIndexMapView<T>>&
                                                  weak_view,
                                              const T& initial) {
    auto view_opt = weak_view.lock();
    if (not view_opt.has_value()) {
