...
TextureVector::TextureVector(const std::shared_ptr<GladGLContext>& ctx,
                             GLsizei levels,
...
GLsizeiptr TextureVector::size() const { return _size; }
...
GLsizeiptr TextureVector::capacity() const { return _cap; }
...
void TextureVector::insert(GLsizeiptr i, const Image& value) { insert_range(i, &value, 1); }
...
void TextureVector::push_back(const Image& value) { insert_range(_size, &value, 1); }
...
void* TextureVector::_alloc(size_t size) { return malloc(size); }

void TextureVector::_free(void* ptr) { free(ptr); }