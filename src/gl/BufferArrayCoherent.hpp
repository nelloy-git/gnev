...
private:
    static constexpr GLbitfield _storage_flags =
...
template <typename T>
BufferArrayCoherent<T>::BufferArrayCoherent(const GladCtx& ctx,
                                            const std::initializer_list<T>& data)
    : BufferArray<T>(ctx, data.size(), data.begin(), _storage_flags)
    , _map(static_cast<T*>(
          BufferArray<T>::glMapBufferRange(0, data.size() * sizeof(T), _storage_flags))) {}
...
void BufferArrayCoherent<T>::set_range(GLsizeiptr i, GLsizeiptr count, const T* data) {
    if (i < 0 || i + count >= BufferArray<T>::size()) {
        throw std::out_of_range("");
    }
