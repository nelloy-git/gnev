...
#include "mesh/base/VertexAttributeType.hpp"

namespace gnev {

struct VertexAttribInfo {
    constexpr VertexAttribInfo(GLuint elements, GLenum type, bool normalized)
        : elements(elements)
        , type(type)
...
constexpr bool VertexAttribInfo::is_signed_type(GLenum type) {
    switch (type) {
    case GL_FLOAT:
    case GL_INT:
    case GL_BYTE:
...
constexpr size_t VertexAttribInfo::get_type_size(GLuint elements, GLenum type) {
    switch (type) {
    case GL_FLOAT:
