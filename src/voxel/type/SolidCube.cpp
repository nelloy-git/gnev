...
VoxelTypeSolidCube::IndexArray
VoxelTypeSolidCube::get_indices(const VoxelRectInfo& rect, size_type base_index) {
    return {base_index, base_index + 1, base_index + 2, base_index + 2, base_index + 3, base_index};
}

VoxelTypeSolidCube::PosArray VoxelTypeSolidCube::get_position(const VoxelRectInfo& rect) {
    // std::cout << "get_position: {" << rect.x << ", " << rect.y << ", " <<
    // rect.z << "}, "
    ...
    case VoxelSide::Top:
        return {
            ...
            std::array<float, 3>{float(rect.x + rect.size_x),
                                 float(rect.y) + 1,
                                 float(rect.z + rect.size_z)},
    ...
    case VoxelSide::Front:
        return {
            ...
            std::array<float, 3>{float(rect.x) + 1,
                                 float(rect.y + rect.size_y),
                                 float(rect.z + rect.size_z)},
    ...
    case VoxelSide::Back:
        return {
            std::array<float, 3>{float(rect.x), float(rect.y), float(rect.z + rect.size_z)},
            std::array<float, 3>{float(rect.x),
                                 float(rect.y + rect.size_y),
                                 float(rect.z + rect.size_z)},
...
int VoxelTypeSolidCube::get_material_id(const VoxelRectInfo& rect) {
    ...
    case VoxelSide::Bottom:
        return bottom_material_id;
    case VoxelSide::Front:
        return front_material_id;
    case VoxelSide::Back:
        return back_material_id;
    case VoxelSide::Right:
        return right_material_id;
    case VoxelSide::Left:
        return left_material_id;
