...
const std::vector<VoxelRect>& VoxelSideMap::get_rectangles() const
{
    return _rects;
}
    ...
    for (size_type x = 0; x < _map.size_x; ++x){
        for (size_type y = 0; y < _map.size_y; ++y){
            for (size_type z = 0; z < _map.size_z; ++z){
                ...
                _map(x, y, z) = (!p_voxel || !_is_visible(voxels, *p_voxel, x, y, z)) ? nullptr : p_voxel;
            }
        }
    }
}
    ...
    for (size_type x = 0; x < _map.size_x; ++x){
        for (size_type y = 0; y < _map.size_y; ++y){
            for (size_type z = 0; z < _map.size_z; ++z){
                ...
                VoxelRect rect {
                    .user_data = {.side = side, .type = p_type},
                    .x = x,
                    .y = y,
                    .z = z,
                };
...
void VoxelSideMap::_search_rect_width(Array3d<uint8_t>& passed, VoxelRect& rect) const
{
    size_type width = 1;
    switch (side)
    {
    ...
    case VoxelSide::Bottom:
        ...
        while(rect.z + width < _map.size_z){
            if (rect.user_data.type != _map(rect.x, rect.y, rect.z + width) || passed(rect.x, rect.y, rect.z + width)){
                break;
            }
    ...
    case VoxelSide::Right:
        ...
        while(rect.x + width < _map.size_x){
            if (rect.user_data.type != _map(rect.x + width, rect.y, rect.z) || passed(rect.x + width, rect.y, rect.z)){
                break;
            }
...
void VoxelSideMap::_search_rect_height(Array3d<uint8_t>& passed, VoxelRect& rect) const
{
    ...
    case VoxelSide::Bottom:
        ...
        while(rect.x + height + 1 < _map.size_x){
            ...
            for (size_type width = 0; width < rect.size_z; ++width){
                ...
                //           << std::endl
                //           << "\tbase: " << rect.user_data.type.get() << " cmp: " << _map(rect.x + height, rect.y, rect.z + width).get()
                //           << std::endl;                

                if (rect.user_data.type != _map(rect.x + height, rect.y, rect.z + width)
    ...
    case VoxelSide::Back:
        ...
        while(rect.y + height < _map.size_y){
            bool same_type = true;
            for (size_type width = 0; width < rect.size_z; ++width){
                if (rect.user_data.type != _map(rect.x, rect.y + height, rect.z + width)
    ...
    case VoxelSide::Right:
        ...
        while(rect.y + height < _map.size_y){
            ...
            for (size_type width = 0; width < rect.size_x; ++width){
                if (rect.user_data.type != _map(rect.x + width, rect.y + height, rect.z)
...
bool VoxelSideMap::_is_visible(const Array3d<std::shared_ptr<VoxelType>>& voxels,
                               const VoxelType& type, size_type x, size_type y, size_type z) const
{
